 ...
 *
 * @package    makani_Real_Estate
 * @subpackage makani_Real_Estate/admin
 * @author     M Saqib Sarwar <saqib@makanithemes.com>
 */
...
class Additional_Details_Meta_Box {
     ...
     */
	private $nonce = 'makani_additional_details_nonce';
	...
	*/
	...
	public function add_additional_details_meta_box() {
		...
		add_meta_box( 'additional-details-meta-box', __( 'Additional Details', 'makani-real-estate' ), array( $this, 'display_additional_details' ), 'property', 'normal', 'core' );
	...
	public function display_additional_details( $post ) {
		...
		/* $detail_titles = get_post_meta($post->ID,'makani_detail_titles',true);
		if( !empty( $detail_titles ) ) {
			$detail_values = get_post_meta($post->ID, 'makani_detail_values', true);
			if (!empty($detail_values)) {
				$additional_details = array_combine( $detail_titles, $detail_values );
				if ( update_post_meta( $post->ID, 'makani_additional_details', $additional_details ) ){
					delete_post_meta( $post->ID, 'makani_detail_titles' );
					delete_post_meta( $post->ID, 'makani_detail_values' );
				}
			}
		} */

		// nonce field for better security
		wp_nonce_field( 'makani_additional_details_meta_box', $this->nonce );

		?>
		<div class="makani-details-wrapper">

			<div class="makani-detail labels">
				<div class="makani-detail-control">&nbsp;</div>
				<div class="makani-detail-title"><label><?php _e( 'Title', 'makani-real-estate' ) ?></label></div>
				<div class="makani-detail-value"><label><?php _e( 'Value', 'makani-real-estate' ); ?></label></div>
				<div class="makani-detail-control">&nbsp;</div>
			</div>
			...
			<div id="makani-additional-details-container">
				...
				$additional_details = get_post_meta( $post->ID, 'makani_additional_details', true );

				if( ! empty ( $additional_details ) ) {

					foreach( $additional_details as $title => $value ) {
						?>
						<div class="makani-detail inputs">
		...
		if( $this->user_can_save( $post_id, $this->nonce ) ) {
			...
			if( isset( $_POST['detail-titles'] ) && isset( $_POST['detail-values'] ) ) {
				...
				if( !empty( $additional_details_titles ) && !empty( $additional_details_values ) ) {
                    ...
                    if( ! empty( $additional_details ) ) {
                        update_post_meta( $post_id, 'makani_additional_details', $additional_details );
                        return;
                    }
				}
			}
            ...
            delete_post_meta( $post_id, 'makani_additional_details' );
		}
	...
	function user_can_save( $post_id, $nonce ) {
	    ...
	    $is_valid_nonce = ( isset( $_POST[ $nonce ] ) && wp_verify_nonce( $_POST[ $nonce ], 'makani_additional_details_meta_box' ) );
